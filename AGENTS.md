...
- **技术文档**：`Context7`（如可用，先 `resolve-library-id` 后 `query-docs`/获取文档）
- **网页搜索**：使用自带的网页搜索功能
- **GitHub 文档**：`DeepWiki`（如可用）

### 💭 分析规划

- **深度思考**：`Sequential-Thinking`（如可用，规划前必须执行）
- **知识管理**：`Memory`（如可用，读取约束，存储决策；如环境支持）
...
## 5.1) 🤝 双子 Agent 协作机制（代码审阅 + 功能测试）

- 当用户明确要求“开启子 agent 协作”、或任务进入**中等及以上复杂度**且涉及多文件改动、编排主流程时，默认启用**两个子 agent 并行协作**。
- 子 agent 的固定分工如下：
  - **审阅 agent**：只做代码审阅，重点检查功能是否真正实现、逻辑是否自洽、是否满足/ 需求文档 / 验收标准，输出按严重度排序的 findings。
  - **测试 agent**：只做功能测试与测试脚本补充，优先补可执行的小范围单元测试 / 集成测试；若需新增测试文件，**统一放在 `test/` 文件夹下**。
- 若用户明确指定模型，必须遵守；若未指定且环境支持，优先使用较强模型执行这两个子任务。
- 主 agent 的职责不可下放：
  - 负责定义本轮目标、拆分子任务、整合两个子 agent 的反馈
  - 负责真正修改主代码、修复漏洞、补齐缺口
  - 负责在收到审阅/测试反馈后继续迭代，直到本轮目标完成或明确阻塞
- 协作流程固定为：
  1. 主 agent 先阅读上下文并锁定本轮目标
  2. 并行启动“审阅 agent + 测试 agent”
  3. 主 agent 在本地继续实现，不等待子 agent 空转
  4. 收到审阅结果后，优先修复高严重度问题
  5. 收到测试结果后，优先补测试缺口、修复失败用例
  6. 最终由主 agent 汇总：已完成内容、未完成内容、验证结果、剩余风险
- 输出要求：
  - 不得只转述子 agent 结论，必须结合主线代码变更进行核实
  - 若子 agent 结论与当前代码状态不一致，必须以**实际工作区内容**为准
  - 若子 agent 建议会扩大范围，主 agent 需先判断是否属于本轮目标，避免无边界扩散
